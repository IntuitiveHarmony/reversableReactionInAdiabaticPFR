...
function SVG_Graph(options) {
  ...
  const xOpts =
    typeof options === "undefined"
      ? {}
      : typeof options.axes === "undefined"
      ? {}
      : typeof options.axes.x === "undefined"
      ? {}
      : options.axes.x;
  const yOpts =
    typeof options === "undefined"
      ...
      : typeof options.axes === "undefined"
      ? {}
      : typeof options.axes.y === "undefined"
      ? {}
      : options.axes.y;
  ...
  this.SVG.style.width = `calc(100% - ${
    this.options.padding[0][0] + this.options.padding[0][1]
  }px)`;
  this.SVG.style.height = `calc(100% - ${
    this.options.padding[1][0] + this.options.padding[1][1]
  }px)`;
  ...
  this.coordToPix = function (x, y) {
    const xPix =
      (100 * (x - this.options.axes.x.range[0])) /
      (this.options.axes.x.range[1] - this.options.axes.x.range[0]);
    const yPix =
      100 -
      (100 * (y - this.options.axes.y.range[0])) /
        (this.options.axes.y.range[1] - this.options.axes.y.range[0]);
    return [xPix, yPix];
  };

  this.pixToCoord = function (x, y) {
    const xCoord =
      this.options.axes.x.range[0] +
      (x / 100) * (this.options.axes.x.range[1] - this.options.axes.x.range[0]);
    const yCoord =
      this.options.axes.y.range[0] +
      ((100 - y) / 100) *
        (this.options.axes.y.range[1] - this.options.axes.y.range[0]);
  ...
  this.createLine = function (options) {
    ...
    if (o.classList.length >= 1) {
      line.classList.add(...o.classList);
    }
    if (o.id !== null) {
      line.id = `${o.id}`;
    }
  ...
  this.createGroup = function (options) {
    ...
    if (o.classList.length >= 1) {
      group.classList.add(...o.classList);
    }
    if (o.id !== null) {
      group.id = `${o.id}`;
    }
  ...
  this.createPath = function (options) {
    ...
    if (o.classList.length >= 1) {
      path.classList.add(...o.classList);
    }
    if (o.id !== null) {
      path.id = `${o.id}`;
    }
  ...
  this.createPoint = function (options) {
    ...
    if (o.classList.length >= 1) {
      point.classList.add(...o.classList);
    }
    if (o.id !== null) {
      point.id = `${o.id}`;
    }
  ...
  this.createText = function (options) {
    ...
    if (o.classList.length >= 1) {
      text.classList.add(...o.classList);
    }
    if (o.id !== null) {
      text.id = `${o.id}`;
    }
    ...
    if (o.alignment[0] === "center") {
      text.style.textAnchor = "middle";
    } else if (o.alignment[0] === "right") {
      text.style.textAnchor = "end";
    }
    if (o.alignment[1] === "center") {
      text.style.transform = `translateY(${o.fontSize / 3}px)`;
    } else if (o.alignment[1] === "top") {
      text.style.transform = `translateY(${(5 * o.fontSize) / 6}px)`;
    }
  ...
  // Create axes group
  const axesGroup = this.createGroup({
    classList: ["svg-axes"],
    parent: this.SVG,
  });

  // Create group for each individual axis
  const topGroup = this.createGroup({
    classList: ["svg-axis", "top"],
    parent: axesGroup,
  });
  const bottomGroup = this.createGroup({
    classList: ["svg-axis", "bottom"],
    parent: axesGroup,
  });
  const leftGroup = this.createGroup({
    classList: ["svg-axis", "left"],
    parent: axesGroup,
  });
  const rightGroup = this.createGroup({
    classList: ["svg-axis", "right"],
    parent: axesGroup,
  });
  ...
  this.redrawAxes = function () {
    ...
    const majorTicksX =
      (this.options.axes.x.range[1] - this.options.axes.x.range[0]) /
      this.options.axes.x.step;
    const majorTicksY =
      (this.options.axes.y.range[1] - this.options.axes.y.range[0]) /
      this.options.axes.y.step;
    ...
    if (this.options.axes.y.display[0]) {
      ...
      for (let i = 0; i <= majorTicksY; i++) {
        ...
        if (i > 0 || this.options.axes.y.showZeroLabel) {
          const majorTickPix = this.coordToPix(
            this.options.axes.x.range[0],
            this.options.axes.y.range[0] + i * this.options.axes.y.step
          );
          ...
          const labelText = Number(
            this.options.axes.y.range[0] + i * this.options.axes.y.step
          ).toFixed(this.options.axes.y.tickLabelPrecision);
        ...
        for (let j = 1; j <= this.options.axes.y.minorTicks; j++) {
          const minorTickPix = this.coordToPix(
            this.options.axes.x.range[0],
            this.options.axes.y.range[0] +
              (i + j / (this.options.axes.y.minorTicks + 1)) *
                this.options.axes.y.step
          );
          if (minorTickPix[1] >= 0) {
    ...
    if (this.options.axes.x.display[0]) {
      ...
      for (let i = 0; i <= majorTicksX; i++) {
        ...
        if (i > 0) {
          const majorTickPix = this.coordToPix(
            this.options.axes.x.range[0] + i * this.options.axes.x.step,
            this.options.axes.y.range[1]
          );
          const majorTick = this.createLine({
            classList: ["major-tick"],
            coord1: majorTickPix,
        ...
        for (let j = 1; j <= this.options.axes.x.minorTicks; j++) {
          const minorTickPix = this.coordToPix(
            this.options.axes.x.range[0] +
              (i + j / (this.options.axes.x.minorTicks + 1)) *
                this.options.axes.x.step,
            this.options.axes.y.range[1]
          );
    ...
    if (this.options.axes.x.display[1]) {
      ...
      for (let i = 0; i <= majorTicksX; i++) {
        ...
        if (i > 0 || this.options.axes.x.showZeroLabel) {
          ...
          const labelText = Number(
            this.options.axes.x.range[0] + i * this.options.axes.x.step
          ).toFixed(this.options.axes.x.tickLabelPrecision);
          const label = document.createElement("div");
    ...
    yAxisLabel.style.transform =
      "translateX(-50%) rotate(-90deg) translateY(10px) translateX(20px)";
  ...
  this.resize = () => {
    const width =
      this.options.parent === document.body
        ? "500px"
        : `${this.options.parent.getBoundingClientRect().width}px`;
    const height =
      this.options.parent === document.body
        ? "500px"
        : `${this.options.parent.getBoundingClientRect().height}px`;
